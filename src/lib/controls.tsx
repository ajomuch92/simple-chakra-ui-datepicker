...
export function MonthSelect({
    ...
    return <Select border='none' value={defaultValue} onChange={onChangeHandler} size='sm' maxW='fit-content'>
        {
            months.map((value, index) => <option value={index} key={index} disabled={isMonthDisabled(index)}>{value}</option>)
        }
    </Select>
}
...
export function YearSelect({defaultValue = new Date().getFullYear(), onChange, endYear = new Date().getFullYear()  + 5, initialYear = new Date().getFullYear()  - 100, maxDate, minDate}: YearSelectProps) {
    const onChangeHandler = (e: ChangeEvent<HTMLSelectElement>) => {
        const { value } = e.target;
        if (onChange) {
            onChange(value as unknown as number);
    ...
    const years = Array.from(Array(endYearComp - initialYearComp + 1).keys(), i => i + initialYearComp);
